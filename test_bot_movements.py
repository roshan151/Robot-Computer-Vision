...
def _encoder_summary(dL: int, dR: int) -> str:
    """Format encoder delta with drift warning.

    dL and dR are signed (negative during reverse moves) because the firmware
    now tracks direction in the encoder counts.  Drift ratio is computed on
    absolute travel so it reads sensibly for both forward and reverse.
    """
    # Signed difference shows which wheel led.
    sync = dL - dR
    # Use absolute travel for the denominator so reverse reads correctly.
    abs_L = abs(dL)
    abs_R = abs(dR)
